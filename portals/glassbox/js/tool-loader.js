...
export async function launchTool(scriptPath, parent, ctx) {
  ...
  try {
    // Always bust module cache — fixed ?v= pins stale tool scripts (e.g. Zip Calculator).
    const href = new URL(`../Scripts/${scriptPath}`, import.meta.url);
    href.searchParams.set("v", String(Date.now()));
    const mod = await import(/* @vite-ignore */ href.href);
